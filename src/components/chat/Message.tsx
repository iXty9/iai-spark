...
import { ChatRecallDialog } from './message/ChatRecallDialog';
      ...
      <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4 animate-fade-in`}>
        <div className={`flex max-w-[85%] sm:max-w-[80%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
          ...
          <div className={`mx-3 ${isUser ? 'text-right' : 'text-left'}`}>
            ...
            {/* Enhanced timestamp with history icon + user download */}
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    ...
                    size="icon"
                    className="h-4 w-4 p-0 text-muted-foreground hover:text-foreground transition-colors"
                    onClick={() => setChatRecallOpen(true)}
                  >
                    <History className="h-3 w-3" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                  <p className="text-xs">Chat Recall</p>
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
