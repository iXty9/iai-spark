...
export async function ensureStorageBucketsExist(): Promise<boolean> {
  try {
    ...
    if (error) {
      logger.error('Error checking storage buckets:', error);
      return false;
    }
    ...
    logger.warn('Avatars bucket does not exist, attempting to create it', { module: 'storage' });
    ...
    if (!result.success) {
      logger.error('Failed to create avatars bucket:', result.error);
      return false;
    }
    ...
    logger.error('Unexpected error in ensureStorageBucketsExist:', error);
    return false;
  }
}
...
export async function uploadAvatar(
  ...
  try {
    ...
    logger.error('Unexpected error in uploadAvatar:', error);
    return { url: null, error };
  }
}
