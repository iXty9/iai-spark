...
class UnifiedThemeService {
  private state: UnifiedThemeState;
  private listeners: Set<(state: UnifiedThemeState) => void> = new Set();
  private originalState: UnifiedThemeState | null = null;
  ...
  private getInitialState(): UnifiedThemeState {
    return {
      mode: 'light',
  ...
  async initialize(userSettings?: ThemeSettings): Promise<void> {
    ...
    try {
      logger.info('Initializing unified theme service', { hasUserSettings: !!userSettings });

      if (userSettings && this.validateThemeSettings(userSettings)) {
        this.state.mode = userSettings.mode || 'light';
        this.state.lightTheme = this.mergeWithDefaults(userSettings.lightTheme || {}, this.getDefaultLightTheme());
        this.state.darkTheme = this.mergeWithDefaults(userSettings.darkTheme || {}, this.getDefaultDarkTheme());
        ...
        try {
          ...
          if (appSettings.default_theme_settings) {
            ...
            if (this.validateThemeSettings(defaultSettings)) {
              this.state.lightTheme = this.mergeWithDefaults(defaultSettings.lightTheme || {}, this.getDefaultLightTheme());
              this.state.darkTheme = this.mergeWithDefaults(defaultSettings.darkTheme || {}, this.getDefaultDarkTheme());
      ...
      logger.info('Unified theme service initialized successfully');
    } catch (error) {
  ...
  // State access
  getState(): UnifiedThemeState {
    return { ...this.state };
  }

  subscribe(listener: (state: UnifiedThemeState) => void): () => void {
    this.listeners.add(listener);
    return () => this.listeners.delete(listener);
  }

  // Core theme operations (immediate application)
  ...
  setBackgroundImage(image: string | null): void {
    ...
    if (!this.state.isInPreview) {
      applyBackgroundImage(image, this.state.backgroundOpacity);
    }
    this.notifyListeners();
  }
  ...
  // Preview mode operations (for settings UI)
  ...
  // Get current theme colors (considering preview state)
  getCurrentTheme(): ThemeColors {
    if (this.state.isInPreview) {
      return this.getCurrentPreviewTheme();
    }
    return this.state.mode === 'dark' ? this.state.darkTheme : this.state.lightTheme;
  }
}
